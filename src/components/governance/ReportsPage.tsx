    ...
    <div id="reports-page" className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wide">
            ...
            <span>Cadastral Analytics & Reporting</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 mt-1">
            Digitization Reports & Velocity
          </h1>
          <p className="text-xs sm:text-sm text-slate-500">
            District-wide land record digitization progress, GIS spatial accuracy metrics, and verification throughput.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <div className="p-5 bg-white rounded-xl border border-slate-200 shadow-xs">
          <span className="text-xs font-medium text-slate-500 block">Digitization Progress</span>
        ...
        <div className="p-5 bg-white rounded-xl border border-slate-200 shadow-xs">
          <span className="text-xs font-medium text-slate-500 block">Certified Records Issued</span>
          <span className="text-3xl font-bold text-emerald-800 mt-1 block">{approved}</span>
          <span className="text-xs text-emerald-600 mt-2 block font-medium">100% Cryptographic Tamper-Proof</span>
        </div>

        <div className="p-5 bg-white rounded-xl border border-slate-200 shadow-xs">
          <span className="text-xs font-medium text-slate-500 block">Spatial Conflict Rate</span>
          <span className="text-3xl font-bold text-amber-800 mt-1 block">2.1%</span>
          <span className="text-xs text-amber-600 mt-2 block font-medium">Down from 14.8% baseline</span>
        </div>

        <div className="p-5 bg-white rounded-xl border border-slate-200 shadow-xs">
          <span className="text-xs font-medium text-slate-500 block">Avg. Officer Processing Time</span>
        ...
        <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          <div>
            <h3 className="font-bold text-slate-900 text-sm sm:text-base">
              Village Cadastre Completion Status
            </h3>
            <span className="text-xs text-slate-500">Sub-District: Hoskote Taluk</span>
          </div>
          <button
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold uppercase text-slate-500">
              ...
              <th className="py-3 px-3">Total Registered Parcels</th>
              <th className="py-3 px-3">Digitized & Verified</th>
          ...
          <tbody className="divide-y divide-slate-100 text-slate-700">
            {villageStats.map((v, i) => (
              <tr key={i} className="hover:bg-slate-50">
                ...
                <td className="py-3 px-3">{v.total} parcels</td>
                <td className="py-3 px-3 text-emerald-700 font-medium">{v.digitized} parcels</td>
